...
function createNAMap_2018(type, upd_data) {
  ...
  function drawNAMap() {
    ...
    if (type == "init") {
      ...
      var status_message = svg.append("text").attr("id", "status_message").attr('x', '50%').attr('y', 10).style('text-anchor', 'middle').style('fill', '#D32F2F').style('font-size', '12px').text('Gathering data, current votes and layout information');
    ...
    function drawElectMap(error, topology, k_topology, pak_prov_topology, pak_topology, na_seats_2018) {
      ...
      if (type == "update") {
        ...
        var upd_seats_list = result_upd.map(function (d) {
          return d.seat;
        });
        var new_data;
        ...
        upd_seats_list.forEach(function (d) {
          new_data.filter(function (f) {
            return f.seat == d;
          })[0].results = result_upd.filter(function (f) {
            return f.seat == d;
          })[0].results;
        });
        ...
        new_data.forEach(function (d) {
          ...
          if (d.results.length == 0) {
            ...
            var appendTimeInfoTo = d3.select('.timeupdates').append('div').classed('timeupdateparent', true);

            appendTimeInfoTo.append('div').classed('updatecircle', true).classed('animated', true).classed('zoomIn', true).append('svg').attr('height', '12.5').attr('width', '12.5').append('circle').attr('cx', '6.25').attr('cy', '6.25').attr('r', '4').attr('fill', 'grey');

            appendTimeInfoTo.append('div').classed('updatetext', true).classed('animated', true).classed('fadeInDefault', true).style('margin-bottom', '5px').append('span').html(function () {
      ...
      if (type == "update") {
        ...
        var weight = function weight(arr_in) {
        ...
        u.enter().append('g').attr('class', function (d) {
          return d.seat;
        }).classed('naSeat_g', true).append('circle').attr("class", "naSeatCircle").classed('2013', true).classed('namap', true).merge(u).attr('cx', function (d) {
          return d.x;
        }).attr('cy', function (d) {
          return d.y;
      ...
      var voronoi = d3.voronoi().x(function (d) {
        return d.x;
      } /*+ randRange(-1, 1)*/) // with some noise on x and y centers
      .y(function (d) {
        return d.y;
      } /*+ randRange(-1, 1)*/).extent([[0, 0], [width, height]]);
      ...
      function activateMouseOv(d, i) {
        ...
        if (datum.results[0] === undefined) {
          ...
          tooltip.append('div').classed('partyicon', true).html(function () {
      ...
      var parties_legend = ["Pakistan Tehreek-e-Insaf", "MUTTHIDA MAJLIS-E-AMAL PAKISTAN", "Pakistan Muslim League (N)", "Independent", "Pakistan Muslim League", "Pakistan Peoples Party Parliamentarians", "Grand Democratic Alliance", "Muttahida Qaumi Movement Pakistan", "Other"];
      ...
      if (type === "init") {
        ...
        var party_legend_svg = party_legend_div.append("svg").classed("partyLegendSVG", true).attr('width', 280).attr('height', 50);
